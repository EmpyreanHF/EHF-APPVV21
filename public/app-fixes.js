                                ...
                                .onSnapshot(function(snap) {
                                    ...
                                    snap.docChanges().forEach(function(change) {
                                        ...
                                        if (change.type === 'added') {
                                            ...
                                            // Is this a brand new post (within last 10s) or initial load?
                                            var isNewPost = post.createdAt && (Date.now()-new Date(post.createdAt).getTime() < 10000);
                                            if (fc) {
                                                if (isNewPost) { fc.prepend(el); } else { fc.appendChild(el); }
                                            ...
                                            if (post.userId === userState.id && !post.isRetweet) {
                                                ...
                                                if (pd && !pd.querySelector('[data-post-id="'+post.id+'"]')) {
                                                    ...
                                                    if (isNewPost) { pd.prepend(pdClone); } else { pd.appendChild(pdClone); }
                                                }
                                                var pp = document.getElementById('profile-posts-feed');
                                                if (pp && !pp.querySelector('[data-post-id="'+post.id+'"]')) {
                                                    if (isNewPost) { pp.prepend(el.cloneNode(true)); } else { pp.appendChild(el.cloneNode(true)); }
                                                }
                                                // Gallery
                                ...
                                .onSnapshot(function(snap) {
                                    ...
                                    snap.docChanges().forEach(function(change) {
                                        ...
                                        if (change.type === 'added') {
                                            ...
                                            var isNewNews = n.createdAt && (Date.now()-new Date(n.createdAt).getTime() < 10000);
                                ...
                                .onSnapshot(function(snap) {
                                    ...
                                    snap.docChanges().forEach(function(change) {
                                        ...
                                        if (change.type === 'added') {
                                            ...
                                            var isNew = item.createdAt && (Date.now()-new Date(item.createdAt).getTime() < 10000);
                                            // Full grid card
                                ...
                                .onSnapshot(function(snap) {
                                    ...
                                    snap.docChanges().forEach(function(change) {
                                        ...
                                        if (change.type === 'added') {
                                            ...
                                            var isNewReel2 = reel.createdAt && (Date.now()-new Date(reel.createdAt).getTime() < 10000);
                                            // Dashboard slider
                ...
                document.body.addEventListener('submit', async function(e) {
                    ...
                    switch (form.id) {
                            ...
                            (async () => {
                                try {
                                    ...
                                    try {
                                        ...
                                        setTimeout(async () => { try { await fbDb.collection('posts').doc(newPost.dataset.postId).set({ id: newPost.dataset.postId, userId: userState.id, username: userState.fullName || userState.username, text, media: _retryMedia, createdAt: new Date().toISOString() }); } catch(e2) {} }, 3000);
                                    }
                                    // Clear quote state
                            ...
                            (async () => {
                                try {
                                    ...
                                    newCard.innerHTML = `${mediaHTML}
                                        ...
                                        <div class="property-seller-info" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:4px;"><span>by @${userState.username} ${salesType==='escrow'?'<i class="fas fa-check-circle verified-badge-small" title="Escrow Protected"></i>':'<i class="fas fa-exclamation-circle unverified-badge-small" title="Direct Sales"></i>'}</span><span style="font-size:0.72rem;color:var(--text-muted);"><i class="fas fa-clock"></i> ${new Date().toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'})}</span></div>
                                    ...
                                    try {
                                        ...
                                        // FIX: retry failed uploads before writing to Firestore
                                        // so other devices always receive real Cloudinary URLs
                                        const _failedItems = uploadResults.filter(function(r){ return !r.ok && r.file; });
                                        if (_failedItems.length > 0) {
                                            const _retryUrls = await Promise.all(_failedItems.map(function(r) {
                                                return window.uploadToCloudinary(r.file, null)
                                                    .then(function(u){ return (u && !u.startsWith('blob:')) ? u : ''; })
                                                    .catch(function(){ return ''; });
                                            }));
                                            _retryUrls.forEach(function(u, ri) {
                                                if (u) { cloudUrls.push(u); _failedItems[ri].file._cloudUrl = u; }
                                            });
                                        }
                                        // Patch local card media elements to cloud URLs (posting device sees media)
                                        if (newCard) {
                                            cloudUrls.forEach(function(cUrl, ci) {
                                                var _els = newCard.querySelectorAll('video, img');
                                                if (_els[ci]) {
                                                    if (_els[ci].tagName === 'VIDEO') { _els[ci].src = cUrl; }
                                                    else { _els[ci].src = cUrl; }
                                                }
                                            });
                                        }
                                        await fbDb.collection('marketplace_listings').doc(newId).set({
                                            ...
                                            mediaTypes: marketplaceMediaFiles.map(function(f){ return f.type || ''; }),
                                            createdAt: new Date().toISOString()
                                        });
                                        console.log('[Marketplace] ✅ Saved to Firestore:', newId, '— media count:', cloudUrls.length);
                                    } catch(e) {
                        ...
                        case 'reel-upload-form': { 
                             ...
                             // FIX: Capture the file reference BEFORE resetting the form,
                             // then reset immediately so the input is fresh for next upload.
                             // Clone the file to guarantee the reference survives form.reset().
                             const capturedFile = new File([videoFile], videoFile.name, { type: videoFile.type });
                             // Reset the form input right away so subsequent uploads work
                             reelVideoFileInput.value = '';
                             reelCaptionInput.value = '';
                             (async () => {
                                 try {
                                     let videoUrl = URL.createObjectURL(capturedFile); // Preview immediately
                                     ...
                                     // FIX: pass reelId so the slider card can be found and upgraded
                                     const _sliderCard = addReelToDashboardSlider({ url: videoUrl, poster: '', caption, reelId: newId });
                                     rewardUserForAction('CREATE_REEL');
                                     // Re-enable button now so user can start a new upload while this one uploads
                                     if (reelSubmitBtn) reelSubmitBtn.disabled = false;
                                     // Upload to Cloudinary in background using the captured file clone
                                     window.uploadToCloudinary(capturedFile, (pct) => { if (pct % 25 === 0) showNotification(`Reel upload: ${pct}%`, 'info'); })
                                         .then(async (cloudUrl) => {
                                             // cloudUrl is a guaranteed https://res.cloudinary.com/... URL
                                             newReelCard.dataset.videoUrl = cloudUrl;
                                             const cardVideo = newReelCard.querySelector('video');
                                             if (cardVideo) cardVideo.src = cloudUrl;
                                             // FIX: also upgrade the dashboard slider card's video src
                                             // so it streams from Cloudinary, not a revoked blob URL
                                             var _sc = _sliderCard || document.querySelector('#dashboard-reels-slider [data-reel-id="'+newId+'"]');
                                             if (_sc) {
                                                 var _sv = _sc.querySelector('video');
                                                 if (_sv) { _sv.src = cloudUrl; var _ss = _sv.querySelector('source'); if(_ss) _ss.src = cloudUrl; }
                                                 _sc.dataset.reelId = newId;
                                             }
                                             ...
                                             try {
                                                 ...
                                                 showNotification('✅ Reel saved to cloud — visible to everyone!', 'success');
                                             } catch(e) {
                                                 console.warn('[Reel] Firestore save failed:', e.message);
                                                 showNotification('Reel uploaded but feed sync failed. Refresh to retry.', 'warning');
                                             }
                                         })
                                         .catch(e => {
                                             console.warn('[Reel] Cloud upload failed:', e.message);
                                             showNotification('⚠ Reel upload failed — check your connection and try again.', 'error');
                                         });
                                 } catch(err) {
                                ...
                                try {
                                    ...
                                    if (newsMediaFile) {
                                        try {
                                            ...
                                            // Do NOT fall back to blob URL — it would be stored in Firestore
                                            // and be invisible to other devices
                                            mediaUrl = null;
                                            mediaType = null;
                                        }
                                    }
